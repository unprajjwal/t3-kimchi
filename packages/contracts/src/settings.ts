...
export const KimchiSettings = makeProviderSettingsSchema(
  {
    // Off by default (like Grok/Cursor/OpenCode): opt in from Settings while early access.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    binaryPath: makeBinaryPathSetting("kimchi").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Kimchi CLI binary.",
        providerSettingsForm: { placeholder: "kimchi", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(CustomModelSetting).pipe(
      Schema.withDecodingDefault(Effect.succeed([])),
...
export type KimchiSettings = typeof KimchiSettings.Type;
...
export const ServerSettings = Schema.Struct({
  ...
  providers: Schema.Struct({
    ...
    kimchi: KimchiSettings.pipe(Schema.withDecodingDefault(Effect.succeed({}))),
...
const KimchiSettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  binaryPath: Schema.optionalKey(TrimmedString),
  customModels: Schema.optionalKey(Schema.Array(CustomModelSetting)),
});
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  providers: Schema.optionalKey(
    Schema.Struct({
      ...
      kimchi: Schema.optionalKey(KimchiSettingsPatch),
